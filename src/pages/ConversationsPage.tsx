...
import {
  ...
  Paperclip, Smile, Mic, Image, MoreVertical, UserCog, CheckCheck,
  ArrowLeftRight, XCircle, Filter,
} from "lucide-react";
              ...
              <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center shrink-0 relative">
                ...
                <div className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border-2 border-card flex items-center justify-center ${conv.channel === "whatsapp" ? "bg-success" : "bg-chart-5"}`}>
                  {conv.channel === "whatsapp" ? (
                    <MessageCircle className="w-2 h-2 text-success-foreground" />
                  ) : (
                    <Instagram className="w-2 h-2 text-chart-5" />
                  )}
                </div>
              </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="h-14 px-5 flex items-center justify-between border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground">{selectedConversation.channel === "whatsapp" ? "WhatsApp" : "Instagram"} · {selectedConversation.sdr}</p>
            </div>
          </div>
          <div className="flex items-center gap-1">
            <button className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-muted transition-colors" title="Assumir conversa">
              <UserCog className="w-4 h-4 text-muted-foreground" />
            </button>
            <button className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-muted transition-colors" title="Transferir">
              <ArrowLeftRight className="w-4 h-4 text-muted-foreground" />
            </button>
            <button className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-muted transition-colors" title="Marcar resolvido">
              <CheckCheck className="w-4 h-4 text-muted-foreground" />
            </button>
            <button className="w-8 h-8 rounded-md flex items-center justify-center hover:bg-destructive/10 transition-colors" title="Encerrar">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3 scrollbar-thin">
          {mockMessages.map((msg) => (
            <div key={msg.id} className={`flex gap-2 ${msg.from === "sdr" ? "justify-end" : ""}`}>
              ...
              <div className={`rounded-lg px-3 py-2 max-w-[70%] ${msg.from === "lead" ? "bg-muted rounded-tl-none" : "bg-primary/10 rounded-tr-none"}`}>
        ...
        <div className="px-5 py-3 border-t border-border shrink-0">
          <div className="flex items-center gap-2 border border-input rounded-lg p-2">
            <button className="w-8 h-8 rounded flex items-center justify-center hover:bg-muted transition-colors">
            ...
            <button className="h-8 px-3 rounded-md bg-primary text-primary-foreground text-xs font-medium hover:opacity-90 transition-opacity flex items-center gap-1.5">
      ...
      <div className="w-72 border-l border-border flex flex-col shrink-0 overflow-y-auto scrollbar-thin">
        ...
        <div className="p-4 space-y-3">
          <div>
            ...
            <p className="text-sm text-foreground flex items-center gap-1.5"><Phone className="w-3.5 h-3.5 text-muted-foreground" />{selectedConversation.phone}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-0.5">Email</p>
            <p className="text-sm text-foreground flex items-center gap-1.5"><Mail className="w-3.5 h-3.5 text-muted-foreground" />{selectedConversation.email}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground mb-0.5">SDR Responsável</p>
            <p className="text-sm text-foreground">{selectedConversation.sdr}</p>
          </div>
          <div>
        ...
        <div className="p-4 border-t border-border space-y-1.5">
          ...
          <button className="w-full h-8 px-3 rounded-md border border-input text-xs text-muted-foreground hover:bg-muted flex items-center gap-2 transition-colors">
            <UserCog className="w-3.5 h-3.5" /> Atribuir SDR
          </button>
          <button className="w-full h-8 px-3 rounded-md border border-input text-xs text-muted-foreground hover:bg-muted flex items-center gap-2 transition-colors">
            <ArrowLeftRight className="w-3.5 h-3.5" /> Transferir Conversa
          </button>
          <button className="w-full h-8 px-3 rounded-md border border-input text-xs text-muted-foreground hover:bg-muted flex items-center gap-2 transition-colors">
        ...
        <div className="p-4 border-t border-border space-y-2">
          ...
          <div className="p-2.5 rounded-md bg-muted/50 border border-border">
            <p className="text-xs font-medium text-foreground">Mensagem de boas-vindas</p>
            <p className="text-[10px] text-muted-foreground">Ativada · Enviada no primeiro contato</p>
          </div>
          <div className="p-2.5 rounded-md bg-muted/50 border border-border">
            <p className="text-xs font-medium text-foreground">Fora do horário</p>
            <p className="text-[10px] text-muted-foreground">Ativada · 19h às 8h</p>
          </div>
        </div>
      </div>
    </div>
