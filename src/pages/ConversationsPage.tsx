...
export default function ConversationsPage() {
  ...
  const messages = messagesMap[selectedConversation.id] || [];
  ...
  const markResolved = () => {
    ...
    setSelectedConversation(prev => ({ ...prev, status: prev.status === "active" ? "resolved" : "active" }));
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="px-5 py-3 border-t border-border shrink-0">
          <div className="flex items-center gap-2 border border-input rounded-lg p-2">
            ...
            <button onClick={() => toast({ title: "Em breve", description: "Áudio será disponibilizado com a integração de mensageria." })} className="w-8 h-8 rounded flex items-center justify-center hover:bg-muted transition-colors">
            ...
            <button onClick={() => toast({ title: "Em breve", description: "Emojis serão disponibilizados em breve." })} className="w-8 h-8 rounded flex items-center justify-center hover:bg-muted transition-colors">
              <Smile className="w-4 h-4 text-muted-foreground" />
            </button>
            <button onClick={handleSend} disabled={!messageInput.trim()} className="h-8 px-3 rounded-md bg-primary text-primary-foreground text-xs font-medium hover:opacity-90 transition-opacity flex items-center gap-1.5 disabled:opacity-50">
